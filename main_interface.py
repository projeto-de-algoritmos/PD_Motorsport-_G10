...
for i in range(3):
    ...
    peso_label = tk.Label(window, text="Peso {}: ".format(i + 1))
...
capacidade_label.grid(row=3, column=0, padx=5, pady=5)

capacidade_entry = tk.Entry(window, width=10)
capacidade_entry.grid(row=3, column=1)

valor_maximo_label = tk.Label(window, text="Valor máximo:")
valor_maximo_label.grid(row=3, column=2, padx=5, pady=5)

valor_maximo_entry = tk.Entry(window, width=10)
valor_maximo_entry.grid(row=3, column=3)
